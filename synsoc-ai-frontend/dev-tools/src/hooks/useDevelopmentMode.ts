...
export function useDevelopmentMode(options?: { isEditModeActive?: boolean }) {
  // Default false here is safe — DevelopmentMode.tsx always passes the actual state.
  // This default only applies to other callers that don't use edit mode.
  const isEditModeActive = options?.isEditModeActive ?? false
  ...
  const highlightElement = useCallback((element: HTMLElement, type: 'hover' | 'select') => {
    ...
    const borderColor = type === 'hover' ? 'var(--color-interactive)' : '#000000'
    ...
    if (type === 'select') {
      setTimeout(() => {
        ...
        if (highlightBox) {
          highlightBox.style.borderColor = 'var(--color-text-muted)' // lighter gray
        }
  ...
  /**
   * Check if event should be handled in select mode
   * Returns the target element if valid, null otherwise
   */
  const shouldHandleEvent = useCallback((e: Event): HTMLElement | null => {
    // Don't handle during text editing, when not in select mode, or when edit mode is active
    if (!state.isSelectMode || state.isInlineEditing || isEditModeActive) return null

    // Guard against non-Element targets (Document, Text nodes, etc.)
    // Prevents TypeError when calling .id, .closest(), .hasAttribute()
    ...
    return target
  }, [state.isSelectMode, state.isInlineEditing, isEditModeActive])

  // Handle element hover
  const handleMouseOver = useCallback((e: MouseEvent) => {
    const target = shouldHandleEvent(e)
    if (!target) return

    setState(prev => ({ ...prev, hoveredElement: target }))
    highlightElement(target, 'hover')
  }, [shouldHandleEvent, highlightElement])

  // Handle keydown to prevent keyboard navigation/activation
  const handleKeyDown = useCallback((e: KeyboardEvent) => {
    const target = shouldHandleEvent(e)
    if (!target) return

    // Only block Space/Enter on elements that would normally be activated by these keys
    // This prevents breaking custom widgets (video players, scroll containers, etc.)
    if (e.key === 'Enter' || e.key === ' ') {
      const tagName = target.tagName.toLowerCase()
      const isInteractive = tagName === 'a' ||
                           tagName === 'button' ||
      ...
      if (isInteractive) {
        e.preventDefault()
        // Allow propagation - we only want to prevent default browser behavior
      }
    }
  }, [shouldHandleEvent])

  // Handle mousedown to prevent default actions before click
  const handleMouseDown = useCallback((e: MouseEvent) => {
    const target = shouldHandleEvent(e)
    if (!target) return

    // Only preventDefault to block browser defaults (navigation, focus changes)
    // Allow propagation so existing mousedown handlers (drag-and-drop, canvas, etc.) still work
    e.preventDefault()
  }, [shouldHandleEvent])
  ...
  }, [shouldHandleEvent, highlightElement, createElementInfo, calculateEditorPosition])

  // Handle mouse leave
  const handleMouseLeave = useCallback(() => {
    // Don't handle mouse leave during text editing or when not in select mode
    if (!state.isSelectMode || state.isInlineEditing || isEditModeActive) return
  ...
  }, [state.isSelectMode, state.isInlineEditing, isEditModeActive, state.selectedElement, clearHighlight, highlightElement])
  ...
  }, [state.isSelectMode, state.selectedElement, clearHighlight, handleKeyDown, handleMouseOver, handleMouseDown, handleClick, handleMouseLeave, handleScroll, highlightElement])
  ...
  const startInlineEditing = useCallback(() => {
    ...
    element.style.outline = '2px solid var(--color-selection-outline)'  // Green outline for text editing
